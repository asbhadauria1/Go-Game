    ...
    for (int row = 0; row < boardSize; row++)
    {
        ...
        for (int col = 0; col < boardSize; col++)
        {
            ...
            if (board[row][col] == EMPTY)
                ...
                cout << "@"; // for displaying white stones
            else
                cout << "o"; // for displaying black stones
            cout << "|";
...
bool isCaptured(int boardSize, const char board[][N], int row, int col, char color, bool visit[][N])
{
    ...
    int cnt = 0;
    bool temp[N][N] = {};
    countLiberty(boardSize, board, row, col, color, temp, cnt);
    if (!cnt)
    {
        return true;
    }
...
int removeStones(int boardSize, char board[][N], int row, int col, const char color)
{
    ...
    if (row >= boardSize || col >= boardSize || row < 0 || col < 0 || board[row][col] != color)
        return 0;
    int cnt = 0;
    if (board[row][col] == color)
    {
    ...
    return cnt + removeStones(boardSize, board, row + 1, col, color) + removeStones(boardSize, board, row, col + 1, color) + removeStones(boardSize, board, row - 1, col, color) + removeStones(boardSize, board, row, col - 1, color);
}
...
int updateBoard(int boardSize, char board[][N], int row, int col, char color)
{
    ...
    if (inBoardBound(row + 1, col, boardSize) && isCaptured(boardSize, board, row + 1, col, opp_color, visit1))
        cnt += removeStones(boardSize, board, row + 1, col, opp_color);

    if (inBoardBound(row - 1, col, boardSize) && isCaptured(boardSize, board, row - 1, col, opp_color, visit2))
        cnt += removeStones(boardSize, board, row - 1, col, opp_color);

    if (inBoardBound(row, col + 1, boardSize) && isCaptured(boardSize, board, row, col + 1, opp_color, visit3))
        cnt += removeStones(boardSize, board, row, col + 1, opp_color);

    if (inBoardBound(row, col - 1, boardSize) && isCaptured(boardSize, board, row, col - 1, opp_color, visit4))
        cnt += removeStones(boardSize, board, row, col - 1, opp_color);
    return cnt;
}
...
bool isLegalMove(int boardSize, const char board[][N], int row, int col, char color, const char previousBoard[][N])
{
    ...
    if (row >= boardSize || col >= boardSize || row < 0 || col < 0 || board[row][col] != EMPTY)
        return false;
    bool visit[N][N] = {};
    char temp[N][N] = {};
    temp[row][col] = color;
    copyBoard(boardSize, board, temp);
    int captured = updateBoard(boardSize, temp, row, col, color);
    int cnt = 0;
    countLiberty(boardSize, temp, row, col, color, visit, cnt);
    if (!cnt)
    {
        if (!captured)
        {
    ...
    if (sameBoard(boardSize, temp, previousBoard))
        return false;
    return true;
}
